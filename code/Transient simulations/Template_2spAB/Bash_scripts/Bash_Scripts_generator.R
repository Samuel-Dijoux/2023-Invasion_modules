...
Fw <- '2spAB';             # Food web nomenclature, consumer-resource system without invasion
...
for(alpha in c(1, 2, 5, 10)){
  ...
  for(i in 1:40){
    texte <- paste(bloc1$text, paste( paste(Fw,alpha,i, sep='_'), 'sh', sep='.'),
                   bloc2$text, paste("BMRs", alpha, sep='_'),
                   bloc3$text, i,
                   bloc4$text, alpha,
                   bloc5$text, sep="")
    cat(texte, file= paste( paste(Fw,alpha,i, sep='_'), 'sh', sep='.'));
  }
} 
################# Automatised command to delete Bash scripts for all body mass combinations
for(alpha in c(1, 2, 5, 10)){
  ...
  setwd(paste(Path, paste("BMRs", alpha, sep="_"),sep='/'));
  for(i in 1:40){
    fn <- paste( paste(Fw,alpha,i, sep='_'), 'sh', sep='.');
    ## Check its existence
    if (file.exists(fn)){ 
