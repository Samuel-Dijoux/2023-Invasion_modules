...
########################################################################
# Create a path directing where will be created all "BMRs" directory folders. Example: "./Template_TC";
Path <- "ZZ HERE ZZ";

# Create a directory where all the chunks of data are stored: e.g. "Databank" here.
dir1 <- paste(Path, "Databank",sep="/");
########################################################################
Fw <-'TC'             # Food web nomenclature, invasion of predator C in Consumer B-resource A system
...
for(alpha in Alpha){
  for(beta in Beta){
    ...
    setwd(dir1); # Go in the directory to save the complete data tables
...
write.table(Invdata, "TC_POP.txt", dec= ".", sep = "\t", row.names=FALSE, col.names=TRUE)
